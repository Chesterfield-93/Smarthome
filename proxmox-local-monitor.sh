...
TELEGRAM_BOT_TOKEN="7359550774:AAEAL4guFNaHZX3DEi9kuzT5TnOe3LqUev8"
TELEGRAM_CHAT_ID="-4243017540"
HOSTNAME=$(hostname)
...
check_vms_and_containers() {
    ...
    while IFS= read -r line; do
        ...
        if [ "$status" = "running" ]; then
            ...
            if [ ! -z "$cpu_usage" ] && [ "$cpu_usage" -gt "$VM_CPU_THRESHOLD" ]; then
                alerts="${alerts}⚠️ VM ${name} (ID: ${vmid}) - Hohe CPU-Last: ${cpu_usage}%\n"
            fi
            
            if [ ! -z "$mem_usage" ] && [ "$mem_usage" -gt "$VM_RAM_THRESHOLD" ]; then
            ...
            if qm agent "$vmid" ping >/dev/null 2>&1; then
                ...
                if [ ! -z "$disk_usage" ] && [ "$disk_usage" -gt "$VM_STORAGE_THRESHOLD" ]; then
    ...
    while IFS= read -r line; do
        ...
        if [ "$status" = "running" ]; then
            ...
            if [ ! -z "$mem_total" ] && [ ! -z "$mem_used" ] && [ "$mem_total" -gt 0 ]; then
